...
use Alterindonesia\KeycloakGuard\Exceptions\TokenException;
...
class KeycloakGuard implements Guard
{
    ...
    {
        if($this->config['session_sync']) $this->syncWithRealm();
    ...
    private function syncWithRealm()
    {
        try {
            $data = Http::withHeaders([
                'Authorization' => 'Bearer '.$this->getTokenForRequest()
            ])->get($this->config['keycloak_url'].'/realms/'.$this->config['keycloak_realm'].'/protocol/openid-connect/userinfo');
            if($data->status() != 200) {
